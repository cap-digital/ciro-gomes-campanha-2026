...
export default async function EficienciaPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  return (
    <div className="pag" style={{ height: "100%", display: "grid", gridTemplateColumns: "1.35fr 1fr", gridTemplateRows: "auto 1fr", gap: 12, minHeight: 0 }}>
      <div className="emp" style={{ gridColumn: "span 2", display: "grid", gridTemplateColumns: "repeat(5,1fr)", gap: 12 }}>
      ...
      <Panel>
        ...
        <div style={{ flex: 1, minHeight: 0, overflow: "auto", display: "flex", flexDirection: "column", gap: 7 }}>
          {data.eficRows.map((r, i) => (
            <div key={i} className="linha-tabela" style={{ display: "grid", gridTemplateColumns: "1.5fr 1fr 62px 62px", gap: 10, alignItems: "center", background: "var(--soft)", border: "1px solid var(--line)", borderRadius: 11, padding: "8px 11px" }}>
      ...
      <div className="emp" style={{ display: "grid", gridTemplateRows: "1fr 1fr", gap: 12, minHeight: 0 }}>
