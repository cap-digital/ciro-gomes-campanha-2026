...
export default async function CriativosPage({
  ...
  const ordem = (
    typeof sp.ordem === "string" && ORDEM_IDS.includes(sp.ordem)
      ? sp.ordem
      : "investimento"
  ) as CriativoOrdem;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <ChipLink
          href={buildHref("/criativos", sp, { status: "todos" })}
          active={data.status === "todos"}
        >
        ...
        {data.statusDisponiveis.map((st) => (
          <ChipLink
            key={st}
            href={buildHref("/criativos", sp, { status: st })}
            active={data.status === st}
            accent={st === "ATIVO" ? "#21C46A" : undefined}
          >
        ...
        <FiltroCampanha
          campanhas={data.campanhasDisponiveis}
          atual={data.campanha}
        />
        <span
          style={{
        ...
        {CRIATIVO_ORDENS.map((o) => (
          <ChipLink
            key={o.id}
            href={buildHref("/criativos", sp, { ordem: o.id })}
            active={ordem === o.id}
          >
            {o.label}
          </ChipLink>
        ...
        >
          <ComparativoCriativos
            anuncios={data.paraComparar}
            resultLabel={data.resultLabel}
          />
        </div>
      </div>
      <div style={{ fontSize: 10.5, color: "var(--dim)", marginTop: -4 }}>
        {data.criativos.length} de {data.totalSemFiltro} anúncios
      </div>
      <div
        className="emp"
        style={{
          ...
          >
              ...
              <span
                style={{
                  ...
                  background:
                    c.status === "ATIVO"
                      ? "rgba(33,196,106,.92)"
                      : /PAUSAD|ARQUIV|EXCLU/.test(c.status)
                        ? "rgba(10,18,38,.85)"
                        : "rgba(245,179,1,.9)",
                  color: "#fff",
                }}
            ...
            >
              ...
              >
                ...
                <span title="Reações, comentários, compartilhamentos e salvamentos">
                  {c.interacoes} interações
                </span>
              </div>
              <div
                style={{
