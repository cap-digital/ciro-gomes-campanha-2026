...
export default async function CriativosPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  const data = await getCriativosData(range, ordem, status);
...
{data.criativos.length} de {data.totalSemFiltro} anúncios · clique para abrir a publicação
          </div>
        </div>
      </div>
      <div style={{ flex: 1, minHeight: 0, display: "grid", gridTemplateColumns: "repeat(5,1fr)", gridAutoRows: "minmax(210px,1fr)", gap: 12, overflow: "auto", alignContent: "start" }}>
