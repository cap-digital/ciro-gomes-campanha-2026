...
export default async function DesempenhoPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  return (
    <div className="pag" style={{ height: "100%", display: "grid", gridTemplateColumns: "minmax(0,1fr)", gridTemplateRows: "auto minmax(0,1.3fr) minmax(0,1fr)", gap: 12, minHeight: 0, minWidth: 0 }}>
      <div className="emp" style={{ display: "grid", gridTemplateColumns: "repeat(7,minmax(0,1fr))", gap: 10, minWidth: 0 }}>
      ...
      <Panel>
        <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap", marginBottom: 8, minWidth: 0 }}>
          ...
          <div className="regua-ab" style={{ marginLeft: "auto", minWidth: 0, overflowX: "auto", display: "flex", alignItems: "center", gap: 7 }}>
            <span style={{ fontFamily: "'Inter',sans-serif", fontVariantNumeric: "tabular-nums", fontSize: 9.5, color: "#35D0FF", flexShrink: 0 }}>A</span>
            ...
            <span style={{ fontSize: 10, color: "var(--dim)", flexShrink: 0 }}>vs</span>
            <span style={{ fontFamily: "'Inter',sans-serif", fontVariantNumeric: "tabular-nums", fontSize: 9.5, color: "#F5B301", flexShrink: 0 }}>B</span>
            <div style={{ display: "flex", gap: 4, flexShrink: 0 }}>
              {METRIC_KEYS.map((k) => (
      ...
      <div className="emp" style={{ display: "grid", gridTemplateColumns: "minmax(0,.85fr) minmax(0,2.3fr) minmax(0,.85fr)", gap: 12, minHeight: 0, minWidth: 0 }}>
        ...
        <Panel style={{ padding: "13px 15px" }}>
          <div style={{ display: "flex", alignItems: "baseline", gap: 8, marginBottom: 2 }}>
            <div style={{ fontSize: 10, letterSpacing: ".16em", color: "var(--muted)", textTransform: "uppercase" }}>Interações por criativo</div>
            <div style={{ fontSize: 10, color: "var(--dim)" }}>ordenado por taxa de engajamento</div>
          </div>
          <div style={{ flex: 1, minHeight: 0, overflow: "auto" }}>
            ...
            <table style={{ width: "100%", minWidth: 520, borderCollapse: "collapse", fontFamily: "'Inter',sans-serif", fontVariantNumeric: "tabular-nums" }}>
              <thead>
              ...
              <tbody>
                {data.interCriativo.map((c, i) => (
                  <tr key={i} style={{ background: i % 2 ? "var(--soft)" : "transparent" }}>
                    <td style={{ padding: "6px", fontSize: 11, maxWidth: 0, width: "30%" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 7, minWidth: 0 }}>
                        <span style={{ width: 7, height: 7, borderRadius: 2, background: c.cor, flex: "0 0 auto" }} />
                        <span style={{ whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{c.nome}</span>
                      </div>
                    </td>
                    <td style={{ padding: "6px", fontSize: 10.5, textAlign: "right", color: "var(--muted2)" }}>{compact(c.impressoes)}</td>
                    <td style={{ padding: "6px", fontSize: 10.5, textAlign: "right", color: "var(--muted2)" }}>{num(c.reacoes)}</td>
                    <td style={{ padding: "6px", fontSize: 10.5, textAlign: "right", color: "var(--muted2)" }}>{num(c.comentarios)}</td>
                    <td style={{ padding: "6px", fontSize: 10.5, textAlign: "right", color: "var(--muted2)" }}>{num(c.compartilhamentos)}</td>
                    <td style={{ padding: "6px", fontSize: 10.5, textAlign: "right", color: "var(--muted2)" }}>{num(c.salvos)}</td>
                    <td style={{ padding: "6px", fontSize: 10.5, textAlign: "right", fontWeight: 700, color: i < 3 ? "#4BE08C" : "var(--text)" }}>
                      {pct(c.taxa, 1)}
                    </td>
                  </tr>
                ))}
            ...
            {!data.interCriativo.length && (
              <div style={{ fontSize: 11.5, color: "var(--dim)", textAlign: "center", padding: "16px 0" }}>sem interações no período</div>
            )}
          </div>
          {data.destaqueCriativo && (
            <div style={{ display: "flex", gap: 7, alignItems: "flex-start", paddingTop: 8, borderTop: "1px solid var(--line)", marginTop: 6 }}>
              <span style={{ width: 7, height: 7, borderRadius: 99, background: "#F5B301", marginTop: 5, flex: "0 0 auto" }} />
              <span style={{ fontSize: 10.5, color: "var(--text2)", lineHeight: 1.45 }}>{data.destaqueCriativo}</span>
            </div>
          )}
        </Panel>
        <Panel style={{ padding: "13px 15px" }}>
